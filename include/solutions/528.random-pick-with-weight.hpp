...
class Solution {
public:
    Solution(std::vector<int>& w) : accumulation(w) {
        std::partial_sum(accumulation.begin(), accumulation.end(), accumulation.begin());
    }
    
    int pickIndex() {
        int rnd = std::rand() % accumulation.back();
        return std::upper_bound(accumulation.begin(), accumulation.end(), rnd) - accumulation.begin();
    }
private:
